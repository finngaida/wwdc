...
class LayoutConstants: NSObject {
    ...
    class var pad:Bool { get { return UIDevice.currentDevice().userInterfaceIdiom == .Pad }}
    class var landscapeRatio: CGFloat { get { return 2 }}
    
    // side edges increase on 6 in portrait
    class var sideEdgesPortraitArray: [CGFloat] { get { return [(pad ? 4 : 1) * 3, (pad ? 2 : 1) * 4] }}
    class var sideEdgesPortraitWidthThreshholds: [CGFloat] { get { return [(pad ? 4 : 1) * 400] }}
    class var sideEdgesLandscape: CGFloat { get { return (pad ? 4 : 1) * 3 }}
    ...
    class var keyboardShrunkSizeArray: [CGFloat] { get { return [(pad ? 2 : 1) * 522, (pad ? 2 : 1) * 524] }}
    class var keyboardShrunkSizeWidthThreshholds: [CGFloat] { get { return [(pad ? 2 : 1) * 700] }}
    class var keyboardShrunkSizeBaseWidthThreshhold: CGFloat { get { return (pad ? 2 : 1) * 600 }}
    ...
    class var rowGapLandscape: CGFloat { get { return (pad ? 1.5 : 1) * 7 }}
    
    // key gaps have weird and inconsistent rules
    class var keyGapPortraitNormal: CGFloat { get { return (pad ? 1.5 : 1) * 6 }}
    class var keyGapPortraitSmall: CGFloat { get { return (pad ? 1.5 : 1) * 5 }}
    class var keyGapPortraitNormalThreshhold: CGFloat { get { return (pad ? 1.5 : 1) * 350 }}
    class var keyGapPortraitUncompressThreshhold: CGFloat { get { return (pad ? 1.5 : 1) * 350 }}
    class var keyGapLandscapeNormal: CGFloat { get { return (pad ? 1.5 : 1) * 6 }}
    class var keyGapLandscapeSmall: CGFloat { get { return (pad ? 1.5 : 1) * 5 }}
    // TODO: 5.5 row gap on 5L
    // TODO: wider row gap on 6L
    class var keyCompressedThreshhold: Int { get { return (pad ? 2 : 1) * 11 }}
    ...
    class var lastRowKeyGapPortrait: CGFloat { get { return (pad ? 1.5 : 1) * 6 }}
    class var lastRowKeyGapLandscapeArray: [CGFloat] { get { return [(pad ? 1.5 : 1) * 8, (pad ? 1.5 : 1) * 7, (pad ? 1.5 : 1) * 5] }}
    class var lastRowKeyGapLandscapeWidthThreshholds: [CGFloat] { get { return [(pad ? 1.5 : 1) * 500, (pad ? 1.5 : 1) * 700] }}
...
class KeyboardLayout: NSObject, KeyboardKeyProtocol {
    ...
    func updateKeyCap(key: KeyboardKey, model: Key, fullReset: Bool, uppercase: Bool, characterUppercase: Bool, shiftState: ShiftState) {
        if fullReset {
            ...
            case Key.KeyType.Settings:
                if key.shape == nil {
                    let gearShape = self.getShape(GearShape)
                    key.shape = gearShape
                }
            case Key.KeyType.Hide:
                if key.shape == nil {
                    let hideShape = self.getShape(HideShape)
                    key.shape = hideShape
                }
            default:
                break
    ...
    func getShape(shapeClass: Shape.Type) -> Shape {
        ...
        if shapeClass == ShiftShape.self {
            return ShiftShape(frame: CGRectZero)
        }
        ...
        for (p, page) in model.pages.enumerate() {
            ...
            for (r, row) in page.rows.enumerate() {
                ...
                if self.characterRowHeuristic(row) {
                    frames = self.layoutCharacterRow(row, index: r, page: p, keyWidth: letterKeyWidth, gapWidth: keyGap, frame: frame)
                }
                    
                    // character row with side buttons: shift, backspace, etc.
                else if self.doubleSidedRowHeuristic(row) {
                    frames = self.layoutCharacterWithSidesRow(row, page: p, frame: frame, isLandscape: isLandscape, keyWidth: letterKeyWidth, keyGap: keyGap)
                }
    ...
    func layoutCharacterRow(row: [Key], index: Int, page: Int, keyWidth: CGFloat, gapWidth: CGFloat, frame: CGRect) -> [CGRect] {
        var frames = [CGRect]()
        ...
        for key in row {
            let width = (key.type == .Return) ? keyWidth * 1.5 : keyWidth
            let roundedOrigin = rounded(currentOrigin)
            
            // avoiding rounding errors
            if roundedOrigin + keyWidth > frame.origin.x + frame.width {
                frames.append(CGRectMake(rounded(frame.origin.x + frame.width - width - actualGapWidth / 2), frame.origin.y, width, frame.height))
            }
            else {
    ...
    func layoutCharacterWithSidesRow(row: [Key], page: Int, frame: CGRect, isLandscape: Bool, keyWidth: CGFloat, keyGap: CGFloat) -> [CGRect] {
        var frames = [CGRect]()
