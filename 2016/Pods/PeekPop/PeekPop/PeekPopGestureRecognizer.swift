...
class PeekPopGestureRecognizer: UIGestureRecognizer
{
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent)
    {
        ...
        if let touch = touches.first, context = context where isTouchValid(touch)
        {
            ...
            self.state = (context.delegate.previewingContext(context, viewControllerForLocation: touchLocation, tag: self.view!.tag) != nil) ? .Possible : .Failed
    ...
    func delayedFirstTouch(touch: UITouch) {
        if isTouchValid(touch) {
            ...
            if let context = context {
                let touchLocation = touch.locationInView(self.view)
                peekPopManager.peekPopPossible(context, touchLocation: touchLocation, tag: self.view!.tag)
            }
            peekPopStarted = true
